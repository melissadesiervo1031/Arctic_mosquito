...
popmetrics<- read.csv("C:/Users/Mitzi/Dropbox/Mosquito Science (1)/Data 2011 to 2018/Data modules/Populationmetrics_2_20_19.csv")
...
lnNfecundity1<-ggplot(popmetrics_2, aes(lnDensityliter, Fecundity)) +geom_point()+geom_text(aes(label=Pond),position=position_nudge(y=-4), size=2)+ylab("Potential Fecundity")+ylim(0,100)+xlab("") + mytheme +theme(axis.text.x=element_blank())
lnNmortality2<-ggplot(popmetrics_2, aes(lnDensityliter, mortalityN0)) +geom_point()+geom_text(aes(label=Pond),position=position_nudge(y=-0.005), size=2)+ylab("Larval mortality")+ylim(0,0.17)+xlab("") + mytheme +theme(axis.text.x=element_blank())+geom_smooth(color="black", method='lm', size=0.5, se=FALSE)
lnNR3<-ggplot(popmetrics_2, aes(lnDensityliter,  Rdens)) +geom_point()+geom_text(aes(label=Pond),position=position_nudge(y=-0.1), size=2)+ylab("\nR")+xlab(expression(ln(N[0]))) + mytheme +ylim(-1.5, 1.5)+ geom_hline(yintercept=0, linetype="dashed")+geom_smooth(color="black", method='lm', size=0.5, se=FALSE)+theme(axis.title.x = element_text(margin = margin(t = 1, r = 0, b = 1, l = 0))) 
...
ProductivityR6<-ggplot(popmetrics_2, aes(Productivity, Rdens)) +geom_point()+geom_text(aes(label=Pond),position=position_nudge(y=-0.1), size=2)+ylab("")+ylim(-1.5, 1.5)+xlim(-0.2,1.8)+xlab(bquote(atop(,"Biofilm productivity" ( ~ µg  ~C %.% ~ 360 ~ cm^{-2} %.% ~day^{-1}))))  + mytheme + geom_hline(yintercept=0, linetype="dashed") +theme(axis.text.y=element_blank()) 
