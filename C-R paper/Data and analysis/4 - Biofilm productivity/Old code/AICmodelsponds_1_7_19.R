...
Area_loss<-lmer(BiofilmProd~Area_loss+(1|Pond), REML=F, data=biofilmpondata)
...
sArea_loss<-lmer(BiofilmProd~Area_loss+(1|Pond), REML=F, data=scaledbiofilmponddata)
...
#Productivity model##
sfull<-lmer(BiofilmProd~Perim_1+Area_1+Area_loss+meanDepth+meanDOC+meanNH4+meanN03N02+meanlogTotalP+meanlogConduc+meanDoPerc+meanpH+meanTemp+meanFPOM_adjust+(1|Pond), REML=F, data=scaledbiofilmponddata)
...
sPerimArealoss<-lmer(BiofilmProd~Perim_1+Area_loss+(1|Pond), REML=F, na.action = "na.fail",data=scaledbiofilmponddata)
...
full<-lmer(BiofilmProd~Perim_1+Area_1+Area_loss+meanDepth+meanDOC+meanNH4+meanN03N02+meanlogTotalP+meanlogConduc+meanDoPerc+meanpH+meanTemp+meanFPOM_adjust+(1|Pond), REML=F, na.action = "na.fail",data=biofilmpondata)
...
##r2 for best model##
r.squaredGLMM(sPerimarealossdepth)

##check VIF of the best model##
vif.mer(sPerimarealossdepth)
...
Perimadjustedy<-as.data.frame((0.0081100*(biofilmpondata$Perim_1-mean(biofilmpondata$Perim_1))-yandresids$resids)+mean(biofilmpondata$BiofilmProd))
Arealossadjustedy<-as.data.frame((-0.0004768*(biofilmpondata$Area_loss-mean(biofilmpondata$Area_loss))-yandresids$resids)+mean(biofilmpondata$BiofilmProd))
Depthadjustedy<-as.data.frame((0.0525046*(biofilmpondata$meanDepth-mean(biofilmpondata$meanDepth))-yandresids$resids)+mean(biofilmpondata$BiofilmProd))
Doadjustedy<-as.data.frame((-0.0144279*(biofilmpondata$meanDoPerc-mean(biofilmpondata$meanDoPerc))-yandresids$resids)+mean(biofilmpondata$BiofilmProd))
...
plot(biofilmpondata$Area_loss, alladjustedys$Arealossadjustedy)
plot(biofilmpondata$meanDepth, alladjustedys$Depthadjustedy)
plot(biofilmpondata$meanDoPerc, alladjustedys$Doadjustedy)

xvariables<-biofilmpondata %>% select(Perim_1, Area_loss, meanDepth, meanDoPerc)
