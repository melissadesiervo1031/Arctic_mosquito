...
##plot adjusted ys##
Perimplot<-ggplot(alladjustedys, aes(x=Perim_1, y=Perimadjustedy)) + geom_point(size=1)+xlab(bquote(atop("Pond perimeter" (m^{2}))))+ylab(bquote(atop("Adjusted Biofilm productivity" , ( ~ µg  ~C %*% ~ 360 ~ cm^{-2} %*% ~day^{-1}))))+ggtitle("")+ylim(-0.5, 2.25)+ stat_smooth(method="lm", se=FALSE, color = "black")+ mytheme
...
Arealossplot<-ggplot(alladjustedys, aes(x=(Area_loss_per*100), y=Arealossadjustedy)) + geom_point(size=1)+xlab(bquote(atop("% Area loss")))+ylim(-0.5, 2.25)+ylab("")+ggtitle("")+ mytheme+ stat_smooth(method="lm", se=FALSE, color = "black")+theme(axis.text.y=element_blank())
...
png("C:/Users/Mitzi/Dropbox/Mosquito Science (1)/Data 2011 to 2018/Figures/Adjustygraph22.png", width = 10, height = 3, units = 'in', res = 800)
