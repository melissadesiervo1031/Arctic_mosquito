...
abundanceandbiofilm<- merge(summarybypond,mosabundance,by="Pond")

##first with N0##
abundancebiofilmlogN0<-ggplot(abundanceandbiofilm, aes(x=logN0, y=Cageminnocage, colour=Pond))+ geom_point(size=2)+geom_text(aes(label=Pond),vjust = -0.5)+xlab("ln(N0)")+ylab("Grazing pressure (ug/cm)")+ggtitle("")+ xlim(2.5, 6) +ylim(-3, 24)+ mytheme + theme(legend.position="none")+ geom_smooth(method='lm')
m1<-lm(Cageminnocage~logN0, data=abundanceandbiofilm)


png("C:/Users/Mitzi/Dropbox/Mosquito Science (1)/Data 2011 to 2018/Figures/abundancebiofilmN0log.png", width = 4, height = 4, units = 'in', res = 800)
abundancebiofilmlogN0
dev.off()

##then with 3-4 instars##
abundancebiofilmlog34instars<-ggplot(abundanceandbiofilm, aes(x=Log34instars, y=Cageminnocage, colour=Pond))+ geom_point(size=2)+geom_text(aes(label=Pond),vjust = -0.5)+xlab("ln(N late intars)")+ylab("Grazing pressure (ug/cm)")+xlim(1.5, 4) + ylim(-3, 24) +ggtitle("")+ mytheme + theme(legend.position="none")+ geom_smooth(method='lm')
m2<-lm(Cageminnocage~Log34instars, data=abundanceandbiofilm)

png("C:/Users/Mitzi/Dropbox/Mosquito Science (1)/Data 2011 to 2018/Figures/abundancebiofilmlog34instars.png", width = 4, height = 4, units = 'in', res = 800)
abundancebiofilmlog34instars
dev.off()
