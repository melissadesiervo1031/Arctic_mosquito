...
Biofilmbypond<-ggplot(summarybytreatment111, aes(x=Pond, y=mean, fill=Treatment))+ geom_bar(position=position_dodge(), stat="identity")+geom_errorbar(aes(ymin=mean-Sterror, ymax=mean+Sterror), colour="black", width=0.08,position=position_dodge(0.85)) +xlab("")+ylab(expression("Total Biofilm" (~ µg  ~C / ~ 360 ~ cm^{2})))+mytheme+theme(legend.title=element_blank())+ scale_fill_discrete(labels=c("Early season (13 days)", "Full season exclosure (33 days)", "Full season no exclosure (33 days)"))+ theme(legend.position=c(0.185, 0.87))
...
cageminpregraph<-ggplot(pondcalculations, aes(x=Pond, y=meanCageminpre))+ geom_bar(aes(), stat="identity", width=0.5)+geom_errorbar(aes(ymin=meanCageminpre-SterrorCageminpre, ymax=meanCageminpre+SterrorCageminpre), colour="black", width=0.08) +xlab("")+ylab(bquote(atop("Biofilm productivity with grazers excluded " , ( ~ µg  ~C %*% ~ 360 ~ cm^{2} ^{-1} %*% ~20 ~days^{-1}))))+ geom_hline(yintercept = 0) +mytheme2+ theme(legend.position="none")

##used bquote to add the line break##
...
##grazing pressure##
cageminnocageraph<-ggplot(pondcalculations, aes(x=Pond, y=meanCageminnocage))+ geom_bar(aes(), stat="identity", width=0.5)+geom_errorbar(aes(ymin=meanCageminnocage-SterrorCageminnocage, ymax=meanCageminnocage+SterrorCageminnocage), colour="black", width=0.08) +xlab("")+ylab(bquote(atop("Grazing pressure: Exclosure - no Exclosure " , ( ~ µg  ~C %*% ~ 360 ~ cm^{2}  ^{-1}%*% ~20 ~days^{-1}))))+ geom_hline(yintercept = 0) +mytheme2+ theme(legend.position="none")
...
grazingpressuregraph<-ggplot(biofilmpredictors, aes(x=threefour, y=Cageminnocage))+ geom_point(size=2)+geom_text(aes(label=Pond),vjust = -0.5)+xlab("log(N late instars/ liter)")+ylab(bquote(atop("Grazing pressure: Exclosure - no Exclosure " , ( ~ µg  ~C %*% ~ 360 ~ cm^{2}  ^{-1}%*% ~20 ~days^{-1}))))+ggtitle("")+ mytheme + theme(legend.position="none")+xlim(1,3.5)+ylim(-2,22)

grazingpressuremodel<-lm(Cageminnocage~threefour, data=biofilmpredictors)
...
biofilmpredictors2<-cbind(biofilmpredictors, as.data.frame(pondcalculations$SterrorCageminnocage))

grazingpressuregrapherror<-ggplot(biofilmpredictors2, aes(x=threefour, y=Cageminnocage))+ geom_point(size=2)+geom_errorbar(aes(ymin=Cageminnocage-pondcalculations$SterrorCageminnocage, ymax=Cageminnocage+pondcalculations$SterrorCageminnocage))+geom_errorbarh(aes(xmin=threefour-threefoursterror, xmax=threefour+threefoursterror))+geom_text(aes(label=Pond),vjust = -0.5)+xlab("log(N late instars/ liter)")+ylab(bquote(atop("Grazing pressure: Exclosure - no Exclosure " , ( ~ µg  ~C %*% ~ 360 ~ cm^{2}  ^{-1}%*% ~20 ~days^{-1}))))+ggtitle("")+ mytheme + theme(legend.position="none")+xlim(0.5,3.5)+ylim(-3,32)

png("C:/Users/Mitzi/Dropbox/Mosquito Science (1)/Data 2011 to 2018/Figures/grazingpressuregrapherrorbars.png", width = 6, height = 5, units = 'in', res = 800)
grazingpressuregrapherror
dev.off()
...
biofilmbysize<-ggplot(biofilmpredictorswing, aes(x=summarysizebypond$meanwing, y=Cageminpre))+ geom_point(size=2)+geom_text(aes(label=Pond),vjust = -0.5)+xlab("Average wing length (mm)")+ylab(bquote(atop("Biofilm productivity with grazers excluded " , ( ~ µg  ~C %*% ~ 360 ~ cm^{2} ^{-1} %*% ~20 ~days^{-1}))))+ggtitle("")+ mytheme + theme(legend.position="none")+ xlim(3.5,5.75)+ylim(-5,40)

biofilmbysize2<-ggplot(biofilmpredictorswing, aes(x=Cageminpre, y=summarysizebypond$meanwing))+ geom_point(size=2)+geom_text(aes(label=Pond),vjust = -0.5)+ylab("Average wing length (mm)")+xlab(bquote(atop("Biofilm productivity with grazers excluded " , ( ~ µg  ~C %*% ~ 360 ~ cm^{2} ^{-1} %*% ~20 ~days^{-1}))))+ggtitle("")+ mytheme + theme(legend.position="none")+ ylim(3.75,5.5)+xlim(-8,42)


biofilmsizemodel<-lm(Cageminpre~summarysizebypond$meanwing, data=biofilmpredictorswing)
anova(biofilmsizemodel)

biofilmsizemodel2<-lm(summarysizebypond$meanwing~Cageminpre, data=biofilmpredictorswing)
anova(biofilmsizemodel)

biofilmsizeabundancemodel<-lm(summarysizebypond$meanwing~Cageminpre*threefour, data=biofilmpredictorswing)
...
png("C:/Users/Mitzi/Dropbox/Mosquito Science (1)/Data 2011 to 2018/Figures/biofilmbysize2.png", width = 6, height = 5, units = 'in', res = 800)
biofilmbysize2
dev.off()
