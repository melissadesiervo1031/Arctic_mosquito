...
AllpondsGDD1<-merge(AllpondsGDD,NoOilGDD, by="DateTime", all = T)
...
allpondsdailythermalsumsDD0[allpondsdailythermalsumsDD0 == 0] <- NA

Thermalsums$OilTS0 <- Thermalsums$OilTS0[!is.na(Thermalsums$OilTS0), ]

Thermalsums<-ggplot(allpondsdailythermalsumsDD0, aes(Date)) + geom_line(aes(y = OilTS0 , colour = "OilTS0 ")) + geom_line(aes(y = NoOilTS0 , colour = "NoOilTS0"), na.rm=TRUE)+ geom_line(aes(y = EastTS0 , colour = "EastTS0 "))+ geom_line(aes(y = IceTS0 , colour = "IceTS0 "))+ geom_line(aes(y =VulgTS0 , colour = "VulgTS0 "))+ geom_line(aes(y = VulgsmallTS0 , colour = "VulgsmallTS0 "))+ geom_line(aes(y = WaterfallTS0 , colour = "WaterfallTS0 ")) + geom_line(aes(y = GolfTS0 , colour = "GolfTS0"))+scale_y_continuous(limits = c(0, 400))+ scale_x_date(date_breaks = "5 days",labels=date_format("%m-%d"),limits = as.Date(c('2018-05-07','2018-07-04')))+mytheme+ylab("Thermal sum (Base > 0)")+theme(legend.title=element_blank())+ geom_hline(yintercept=300, linetype="dashed")+scale_color_hue(labels = c("East","Golf","Ice","NoOil","Oil","Vulgsmall","Vulgaris","Waterfall")) 
